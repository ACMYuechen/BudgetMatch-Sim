...
- 快照：synthetic-products-v1；策略：offline-keyword-rule-v1；代码标记：cb5512a+M2.1-worktree（调用者提供）。
...
| 指标 | 结果 |
| --- | --- |
| 完成结果硬约束违规率 | 0/48（0.00%） |
| 预期终态/错误类别命中 | 62/64（96.88%） |
| 可满足任务成功率 | 24/40（60.00%） |
| 需求覆盖率（含不可满足需求） | 33/61（54.10%） |
| Recall@K（micro） | 91/92（98.91%） |
| 完成结果事实一致性 | 48/48（100.00%） |
| 规则兜底率（含显式故障注入） | 2/64（3.12%） |
| 成功轮次重放通过率 | 48/48（100.00%） |
| 持久化轮次数符合预期 | 64/64（100.00%） |

总请求 64，完成 48，违规请求 0，无相关 SKU 标注 22；商品查询 50 次，故障替身调用 2 次，模型调用 0 次。Token/费用、真实向量召回、解释文本评审均未测量。

本地端到端 P50=0.3119 ms，P95=0.9909 ms（nearest-rank，小样本且无外部依赖，仅作本机参考）。安全/终态/幂等门禁：false；该门禁不代表推荐质量达标。

## 固定划分

| 划分 | 用例数 | 可满足成功 | 覆盖率 | Recall@K |
| --- | --- | --- | --- | --- |
| dev | 31 | 11/21（52.38%） | 15/31（48.39%） | 53/54（98.15%） |
...
| multi_turn | 8 | 6/7（85.71%） | 6/8（75.00%） |
...
| history_budget_down | dev | rejected | 0/1 |  |  |
| history_unsatisfiable_after_cut | dev | rejected | 0/1 |  |  |
