...
// 同时保留 Request 结构体 + NewContext/FromContext 用于批量存取，兼容 gRPC/HTTP 拦截器。
package request
...
// NewContext 将 Request 中的所有字段批量注入 Context。
//
// 该函数供 gRPC/HTTP 拦截器在解析完请求后使用，写入时会复制 Request 避免外部修改。
func NewContext(ctx context.Context, req *Request) context.Context {
	if req == nil {
		return ctx
	}
	cloned := cloneRequest(req)
	ctx = WithToken(ctx, cloned.Token)
	ctx = WithUserID(ctx, cloned.UserID)
	ctx = WithRole(ctx, cloned.Role)
	ctx = WithRequestID(ctx, cloned.RequestID)
	ctx = WithClientIP(ctx, cloned.ClientIP)
	ctx = WithUserAgent(ctx, cloned.UserAgent)
	ctx = WithHeaders(ctx, cloned.Headers)
	return ctx
}

// FromContext 从 Context 批量读取所有字段，组装为 *Request。
//
// 返回值是新分配的 Request，业务代码修改副本不会影响 Context 内数据。
func FromContext(ctx context.Context) (*Request, bool) {
	req := &Request{
		...
		ClientIP:  TryClientIP(ctx),
		UserAgent: TryUserAgent(ctx),
		Headers:   cloneAllowedHeaders(TryHeaders(ctx)),
	}
	return req, true
}

// MustFromContext 从 Context 中读取请求信息，不存在时触发 panic。
func MustFromContext(ctx context.Context) *Request {
	req, ok := FromContext(ctx)
	if !ok {
		panic(errors.RequestContextNotFound)
	}
	return req
}
...
// ──────────────── 兼容旧版 getter（供 interceptor 等现有调用方）───────────

// Token 返回 Context 中的 Token，缺失时返回空字符串。
// 等价于 TryToken。
func Token(ctx context.Context) string { return TryToken(ctx) }

// RequireToken 返回 Context 中的 Token，缺失时返回错误。
// 等价于 MustToken。
func RequireToken(ctx context.Context) (string, error) { return MustToken(ctx) }

// UserID 返回 Context 中的用户 ID，缺失时返回空字符串。
// 等价于 TryUserID。
func UserID(ctx context.Context) string { return TryUserID(ctx) }

// RequireUserID 返回 Context 中的用户 ID，缺失时返回错误。
// 等价于 MustUserID。
func RequireUserID(ctx context.Context) (string, error) { return MustUserID(ctx) }

// Role 返回 Context 中的用户角色，缺失时返回 0。
// 等价于 TryRole。
func Role(ctx context.Context) int64 { return TryRole(ctx) }

// RequestID 返回 Context 中的请求 ID，缺失时返回空字符串。
// 等价于 TryRequestID。
func RequestID(ctx context.Context) string { return TryRequestID(ctx) }

// ClientIP 返回 Context 中的客户端 IP，缺失时返回空字符串。
// 等价于 TryClientIP。
func ClientIP(ctx context.Context) string { return TryClientIP(ctx) }

// UserAgent 返回 Context 中的 User-Agent，缺失时返回空字符串。
// 等价于 TryUserAgent。
func UserAgent(ctx context.Context) string { return TryUserAgent(ctx) }

// Header 返回白名单内的单个请求头，名称匹配不区分大小写。
// 等价于 TryHeader。
func Header(ctx context.Context, key string) string { return TryHeader(ctx, key) }

// ──────────────────────── 内部辅助函数 ───────────────────────────────────

// stored 从 Context 中读取所有字段组装为 *Request。
//
// 由 grpc.go 的 NewOutgoingContext 调用，用于提取白名单字段注入 outgoing metadata。
func stored(ctx context.Context) (*Request, bool) {
...
func cloneRequest(req *Request) *Request {
	...
	cloned := *req
	cloned.Headers = cloneAllowedHeaders(req.Headers)
	return &cloned
}
