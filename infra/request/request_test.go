...
func TestMaskToken(t *testing.T) {
	...
	assert.Equal(t, "abcd***", maskToken("abcd"))     // <=8 个字符
	assert.Equal(t, "12345678***", maskToken("12345678")) // =8 个字符
...
func TestFromHTTPRequest_BearerToken(t *testing.T) {
	...
	req, err := FromHTTPRequest(r)
	require.NoError(t, err)
	assert.Equal(t, "my-jwt-token", req.Token)
...
func TestFromHTTPRequest_BearerCaseInsensitive(t *testing.T) {
	for _, prefix := range []string{"Bearer", "bearer", "BEARER", "bEaReR"} {
		...
		req, err := FromHTTPRequest(r)
		require.NoError(t, err)
		assert.Equal(t, "my-token", req.Token, "prefix=%q", prefix)
	}
}
...
func TestFromHTTPRequest_NonBearerToken(t *testing.T) {
	...
	req, err := FromHTTPRequest(r)
	require.NoError(t, err)
	assert.Equal(t, "Basic dXNlcjpwYXNz", req.Token)
}
...
func TestFromHTTPRequest_ClientIP_XRealIP(t *testing.T) {
	...
	req, err := FromHTTPRequest(r)
	require.NoError(t, err)
	assert.Equal(t, "172.16.0.1", req.ClientIP)
}
...
func TestFromHTTPRequest_ClientIP_RemoteAddr(t *testing.T) {
	...
	req, err := FromHTTPRequest(r)
	require.NoError(t, err)
	assert.Equal(t, "10.10.10.10", req.ClientIP)
}
...
func TestFromHTTPRequest_UserAgent(t *testing.T) {
	...
	req, err := FromHTTPRequest(r)
	require.NoError(t, err)
	assert.Equal(t, "Mozilla/5.0 TestBrowser", req.UserAgent)
}
...
func TestFromHTTPRequest_HeadersWhitelist(t *testing.T) {
	...
	req, err := FromHTTPRequest(r)
	require.NoError(t, err)
	assert.Len(t, req.Headers, 3)
	assert.Equal(t, "Bearer tok", req.Headers["Authorization"])
	assert.Equal(t, "rid-1", req.Headers["X-Request-Id"])
	assert.Equal(t, "UA", req.Headers["User-Agent"])
	assert.NotContains(t, req.Headers, "X-Custom")
}

// ──────────────────────── gRPC 解析：FromGRPCContext ─────────────────────

func TestFromGRPCContext_NoMetadata(t *testing.T) {
	req, err := FromGRPCContext(context.Background())
	require.NoError(t, err)
	assert.Empty(t, req.Token)
...
func TestFromGRPCContext_BearerToken(t *testing.T) {
	...
	req, err := FromGRPCContext(ctx)
	require.NoError(t, err)
	assert.Equal(t, "grpc-token", req.Token)
	assert.Equal(t, "greq-1", req.RequestID)
	assert.Equal(t, "grpc-agent/1.0", req.UserAgent)
}
...
func TestFromGRPCContext_NonBearerToken(t *testing.T) {
	...
	req, err := FromGRPCContext(ctx)
	require.NoError(t, err)
	assert.Equal(t, "CustomScheme cred-data", req.Token)
}
...
func TestHTTPToContextRoundTrip(t *testing.T) {
	...
	ctx := context.Background()
	ctx = WithToken(ctx, req.Token)
	ctx = WithRequestID(ctx, req.RequestID)
	ctx = WithClientIP(ctx, req.ClientIP)
	ctx = WithUserAgent(ctx, req.UserAgent)
