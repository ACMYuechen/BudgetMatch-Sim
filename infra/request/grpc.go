...
// singleMetadata 读取 metadata 中指定 key 的唯一值，重复出现时返回 RequestInvalidHeader。
...
// firstMetadata 读取 metadata 中指定 key 的首个值，缺失时返回空字符串。
...
// grpcClientIP 从 gRPC peer 信息中提取客户端 IP，缺失或解析失败时返回空字符串。
