...
// Agent 会话轮次
func AgentConversationTurnsHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {

	return func(w http.ResponseWriter, r *http.Request) {
		var (
			in  = new(types.AgentConversationTurnsReq)
			ctx = r.Context()
		)
		...
		l := agent.NewAgentConversationTurnsLogic(ctx, svcCtx)
		resp, err := l.AgentConversationTurns(in)
		if err != nil {
			httpx.Error(w, err)
		} else {
			httpx.OkJson(w, resp)
