...
// SKU详情，用于推荐结果定位商品
func SkuDetailHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {

	return func(w http.ResponseWriter, r *http.Request) {
		var (
			in  = new(types.MallSkuDetailReq)
			ctx = r.Context()
		)
		...
		l := mall.NewSkuDetailLogic(ctx, svcCtx)
		resp, err := l.SkuDetail(in)
		if err != nil {
			httpx.Error(w, err)
		} else {
			httpx.OkJson(w, resp)
