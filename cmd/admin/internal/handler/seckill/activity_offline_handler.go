...
// 下线活动
func ActivityOfflineHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		var (
			in  = new(types.ActivityOfflineReq)
			ctx = r.Context()
		)
		...
		l := seckill.NewActivityOfflineLogic(ctx, svcCtx)
		resp, err := l.ActivityOffline(in)
		if err != nil {
			httpx.Error(w, err)
		} else {
			httpx.OkJson(w, resp)
