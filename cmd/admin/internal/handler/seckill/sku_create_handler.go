...
// 创建SKU
func SkuCreateHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		var (
			in  = new(types.SkuCreateReq)
			ctx = r.Context()
		)
		...
		l := seckill.NewSkuCreateLogic(ctx, svcCtx)
		resp, err := l.SkuCreate(in)
		if err != nil {
			httpx.Error(w, err)
		} else {
			httpx.OkJson(w, resp)
