...
// 预热活动
func ActivityPreheatHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		var (
			in  = new(types.ActivityPreheatReq)
			ctx = r.Context()
		)
		...
		l := seckill.NewActivityPreheatLogic(ctx, svcCtx)
		resp, err := l.ActivityPreheat(in)
		if err != nil {
			httpx.Error(w, err)
		} else {
			httpx.OkJson(w, resp)
