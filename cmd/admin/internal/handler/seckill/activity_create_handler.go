...
// 创建活动
func ActivityCreateHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		var (
			in  = new(types.ActivityCreateReq)
			ctx = r.Context()
		)
		...
		l := seckill.NewActivityCreateLogic(ctx, svcCtx)
		resp, err := l.ActivityCreate(in)
		if err != nil {
			httpx.Error(w, err)
		} else {
			httpx.OkJson(w, resp)
