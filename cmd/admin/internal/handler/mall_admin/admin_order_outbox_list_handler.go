...
// 订单Outbox事件列表
func AdminOrderOutboxListHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		var (
			in  = new(types.AdminOrderOutboxListReq)
			ctx = r.Context()
		)
		...
		l := mall_admin.NewAdminOrderOutboxListLogic(ctx, svcCtx)
		resp, err := l.AdminOrderOutboxList(in)
		if err != nil {
			httpx.Error(w, err)
		} else {
			httpx.OkJson(w, resp)
