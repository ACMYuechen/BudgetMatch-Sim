...
// 创建SKU
func AdminCreateSkuHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		var (
			in  = new(types.AdminCreateSkuReq)
			ctx = r.Context()
		)
		...
		l := mall_admin.NewAdminCreateSkuLogic(ctx, svcCtx)
		resp, err := l.AdminCreateSku(in)
		if err != nil {
			httpx.Error(w, err)
		} else {
			httpx.OkJson(w, resp)
