...
// SKU详情
func AdminSkuDetailHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		var (
			in  = new(types.AdminSkuDetailReq)
			ctx = r.Context()
		)
		...
		l := mall_admin.NewAdminSkuDetailLogic(ctx, svcCtx)
		resp, err := l.AdminSkuDetail(in)
		if err != nil {
			httpx.Error(w, err)
		} else {
			httpx.OkJson(w, resp)
