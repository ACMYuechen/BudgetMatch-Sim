...
// 订单Outbox事件详情
func AdminOrderOutboxDetailHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		var (
			in  = new(types.AdminOrderOutboxDetailReq)
			ctx = r.Context()
		)
		...
		l := mall_admin.NewAdminOrderOutboxDetailLogic(ctx, svcCtx)
		resp, err := l.AdminOrderOutboxDetail(in)
		if err != nil {
			httpx.Error(w, err)
		} else {
			httpx.OkJson(w, resp)
