...
// 重放订单Outbox死信
func AdminReplayOrderOutboxHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		var (
			in  = new(types.AdminReplayOrderOutboxReq)
			ctx = r.Context()
		)
		...
		l := mall_admin.NewAdminReplayOrderOutboxLogic(ctx, svcCtx)
		resp, err := l.AdminReplayOrderOutbox(in)
		if err != nil {
			httpx.Error(w, err)
		} else {
			httpx.OkJson(w, resp)
