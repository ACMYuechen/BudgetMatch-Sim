...
> 状态：初版需求基线（保留设计目标，不作为当前工作流执行清单）
>
> 初始设计分支：`chore/ci`
>
> 初版日期：2026-08-08；说明核对：2026-09-16
>
> 范围：持续集成（CI），不包含部署（CD）
...
初版设计时尚无前端自动化测试脚本。当前已增加 Playwright 的 `npm run test:e2e`，但 Web Check 仍仅执行上述三项；接入浏览器测试门禁属于后续工作，并非已有 `npm test` 或 Vitest 检查。
