...
## 2026-07-28 请求上下文统一封装（feat/infra/hfs）

### 改了什么

- 新增 `infra/request` 包，统一管理请求身份和常用请求信息。
- 支持 HTTP 请求解析、gRPC metadata 解析及白名单字段透传。
- app/admin 认证中间件和通用 gRPC 拦截器统一使用新的请求上下文。
- 迁移商城、秒杀、用户模块的 `"user_id"` 读取点，并补充核心测试。

### 为什么

- 解决字符串 Context Key 分散、类型断言不统一和跨服务透传规则不一致的问题。

### 影响面

- 影响认证中间件、gRPC 拦截器及相关用户身份读取逻辑。
- 不修改接口、业务错误码和角色规则，Token 校验仍由原认证链路负责。

### 怎么验证的

- 请求上下文、HTTP/gRPC 解析和拦截器测试通过。
- 执行 `go test -timeout 30s ./...` 和 `git diff --check`，均通过。
