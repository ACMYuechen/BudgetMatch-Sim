...
2026-09-20 用户要求普通联调在实际项目数据库保留明确标识的记录，后续纠正目标为远程数据库。此前“已确认本地测试库”的表述是助手错误理解，不再作为执行依据；不得将远程业务连接写入原清理性测试 DSN。授权不包括清库、覆盖原数据、迁移、权限调整、服务重启或付费调用；既有故障测试仍只运行于可丢弃环境。远程网络入口已连通，现有配置凭据认证失败，尚无业务查询或写入；边界见 [开发库联调约定](agent.md#115-开发库联调与记录保留约定)。

M6.3a1 新增独立 `dev-records` 工具，显式选择 `.env` 的单个 DSN 或配置文件的 `Database.DSN`，核对库名与 loopback 目标；预检连接由数据库强制只读，不泄露口令或自动迁移。写入另须指定已有启用账号及独立 run ID，事务内再次核对账号/会话锁，保留带演示标识的两轮历史；不改账号、密码或角色，不接入 LLM/Mall/Embedding。该工具使用操作者显式选择的数据库凭据，**不是 HTTP/RPC 用户鉴权入口，也不应作为远程管理接口暴露**。它仍只支持本机 loopback，没有因用户纠正远程地址而自动放宽目标保护；尚未在实际库写入或验收页面展示，详见 [开发库预检入口](agent.md#116-m63a1-开发库只读预检与保留演示会话入口)。
