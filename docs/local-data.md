...
| 历史 PostgreSQL 测试库 | `127.0.0.1:5432` | `budgetmatch_sim_test` / `budgetmatch_test` 保留，不再配置到业务 `.env` |
...
对业务库只做连接、只读查询及扩展初始化；清理性集成测试只能使用显式选择的可丢弃测试环境，不要求固定库名。移除配置没有删除历史测试库、账号或数据，也没有清空现有业务表、迁移此前 Agent 保留库记录，不代表最新业务代码或完整浏览器流程已在本机上线。
