...
[文档导航](README.md) · [本地数据源](local-data.md) · [生产环境](https://github.com/ACMYuechen/BudgetMatch-Sim-Gitops#部署流程)
...
Compose 应用容器使用内部服务地址，不使用容器内的 `127.0.0.1` 访问宿主服务。细节见[本地连接模式](local-data.md#宿主机与容器模式)。
...
| 支付宝沙箱 | `ALIPAY_APP_ID`、`ALIPAY_SELLER_ID`、`ALIPAY_PRIVATE_KEY`、`ALIPAY_PUBLIC_KEY`、通知/返回 URL | 未配置时支付服务可启动，但支付功能不可用；完整接入见[支付指南](PAYMENT.md) |
...
BGE-M3 使用 `BAAI/bge-m3` 和 1024 维，不能沿用模板的 1536。RAG 还需要 pgvector、Mall 和独立索引身份；模型/维度变化不得自动删表重建。完整示例与索引约束见 [Agent 指南](AGENT.md#模型与-embedding)。

## 测试与部署边界

普通开发不维护常驻测试 DSN；清理性集成测试由测试入口显式提供 `RAG_TEST_PG_DSN` 等连接，不读取业务 `.env` 或回退 `DATABASE_DSN`，见 [CI 指南](CONTRIBUTION.md#go-检查与测试环境)。已删除的旧测试库和账号不需要重新创建。

生产 Secret、镜像与发布配置统一见[私有运维仓库](https://github.com/ACMYuechen/BudgetMatch-Sim-Gitops#环境与数据)。本地配置不复用生产凭据；更新配置不代表 SMTP、OSS、支付或模型已验证可用。
