...
四个阶段的前端实现与模拟 API 验证已完成；另已通过本机真实 Auth/Agent/网关/数据库的登录、推荐与历史恢复联调，限定规则 + Mock 模式。真实商城、支付、模型与部署环境仍待验收，详细边界与浏览器测试命令见 [前端路线图](docs/frontend-roadmap.md)。
