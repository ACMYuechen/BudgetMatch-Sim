...
- M6.3a1 提供单独的 `dev-records` 开发库入口：默认只读预检，显式选择库/已有账号后才保存两轮演示历史；重复执行严格重放，不清库、不迁移、不调用外部模型。按用户确认使用 `.env` 目标，不切换为服务模板中的另一个库；当前目标端口未监听，尚未实际灌入记录或验证页面展示，见 [保留记录的联调入口](docs/agent.md#116-m63a1-开发库只读预检与保留演示会话入口)。
- M6.3a2 补充提交后换连接核对和独立 `-verify-demo` 只读复查；区分提交成功但复查失败与提交结果未知，不自动重写或删除数据。开发库/页面实际联调仍待环境就绪，详见 [提交后确认与只读复查](docs/agent.md#117-m63a2-提交后确认与只读复查)。
