...
- M6.3a2 补充提交后换连接核对和独立 `-verify-demo` 只读复查；区分提交成功但复查失败与提交结果未知，不自动重写或删除数据。失败报告提供脱敏的阶段/原因分类，区分连接、认证和数据库权限问题；开发库/页面实际联调仍待连接目标对齐并就绪，详见 [提交后确认与只读复查](docs/agent.md#117-m63a2-提交后确认与只读复查)。
