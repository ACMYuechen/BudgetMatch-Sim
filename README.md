...
- 接口、会话、评测口径与分步计划统一见 [Agent 开发文档](docs/agent.md)（M2 按用户决定阶段收尾、独立复核后置；M3.2 本地安全子步、M3.3 检索实验/离线回放及过滤回退修复已交付，默认仍为向量优先。M4.1a/b 需求基础与独立规划、M4.2a 有界搜索及 M4.2b 独立演示执行链已实现：`/intent/execute` 消费当前就绪规划，复核后重选并原子保存结果，默认关闭且不接受真实 Mall。规划接口仍不执行推荐，旧推荐入口对规划会话继续拒绝。下一步 M4.2c 贯通真实类别证据与 Mall 核验；旧索引迁移、真实数据库/服务验证、真实检索收益及面向用户的真实推荐流式尚未完成）。
