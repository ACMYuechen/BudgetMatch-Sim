...
- 接口、会话、评测口径与分步计划统一见 [Agent 开发文档](docs/agent.md)（M2 按用户决定阶段收尾、独立复核后置；M3.2 本地安全子步、M3.3 检索实验/离线回放及过滤回退修复已交付，默认仍为向量优先。M4.1a/b 的领域基础与独立需求规划、会话和幂等已实现；M4.2a 已增加内部演示快照的有界 Beam Search 与硬约束复核，尚未接入推荐入口。规划接口仍只保存需求或返回澄清，规划会话暂不能执行旧推荐。下一步 M4.2b 接入约束执行链、最终检查与结果契约。旧索引迁移、真实分类/数据库/服务验证、真实检索收益及面向用户的真实推荐流式尚未完成）。
