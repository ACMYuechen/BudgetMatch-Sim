...
- M6.3a1 提供单独的 `dev-records` 本机开发库入口：默认只读预检，显式选择库/已有账号后才保存两轮演示历史；重复执行严格重放，不清库、不迁移、不调用外部模型。用户已纠正实际联调目标为远程数据库，不能再把 `.env` 的本地测试 DSN 当作业务目标；远程 `5432` 已连通，但现有配置凭据认证失败，未灌入记录或验证页面展示，见 [保留记录的联调入口](docs/agent.md#116-m63a1-开发库只读预检与保留演示会话入口)。
- M6.3a2 补充提交后换连接核对、独立 `-verify-demo` 只读复查及脱敏故障诊断；区分提交成功但复查失败与提交结果未知，不自动重写或删除数据。另提供显式远程授权、地址/库名双重核对、私密配置及 `verify-full` TLS 的[远程联调入口](docs/agent.md#118-m63a2-显式远程目标与-tls-连接保护)，不复用清理性测试 DSN、不降级明文。实际数据库记录/页面仍待有效凭据和安全连接，不代表真实环境验收通过。
