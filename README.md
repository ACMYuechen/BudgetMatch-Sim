...
- 接口、会话、评测口径与分步计划统一见 [Agent 开发文档](docs/agent.md)（M2 按用户决定阶段收尾、独立复核后置；M3.2b1 索引写入协调/模型切换保护及 M3.2b2 Mall 跨页一致快照已完成本地实现，下一步 M3.2c 候选新鲜度；旧索引迁移、真实数据库验证及面向用户的真实推荐流式尚未完成）。
