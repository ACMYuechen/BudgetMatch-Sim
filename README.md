...
- 接口、会话、评测口径与分步计划统一见 [Agent 开发文档](docs/agent.md)（M2 按用户决定收尾、独立复核后置；M3 本地安全/检索实验已交付，默认向量优先。M4.1～M4.3a、M5.1～M5.3b 本地实现及 M6.1 隔离故障验证已交付，M5.3a 推荐页 26 项交互已通过，完整口径见执行记录。`/intent/execute` 默认关闭，未迁移、回填或启用实例，新旧推荐仍拒绝规划会话。下一步 M6.2 独立真实存储/多进程验收，再进行 M6.3 供应商/代理/回滚与最终报告；外部实验需另行授权，真实分类数据、向量效果及独立人工复核仍未完成）。
