...
- 接口、会话、评测口径与分步计划统一见 [Agent 开发文档](docs/agent.md)（M2 按用户决定收尾、独立复核后置；M3 本地安全/检索实验已交付，默认向量优先。M4.1～M4.3a 本地规划/搜索/分类核验/回放与 M5.1～M5.3b 本地流式链路已实现，M5.3a 推荐页 26 项浏览器交互已通过，完整测试口径见执行记录。`/intent/execute` 默认关闭，未迁移、回填或启用实例，新旧推荐仍拒绝规划会话。下一本地步为 M6 隔离故障矩阵与交付材料；真实数据库/服务、分类数据、向量效果、供应商/代理/多实例流式验收仍未完成）。
