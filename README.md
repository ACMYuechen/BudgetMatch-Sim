...
- 新增 `RecommendStream` RPC：用户 JWT、≤30 秒传输 deadline、共享校验/原子保存、重放及取消传播。已接入 Eino Stream、脱敏工具事件和有界背压；独立解释模型只接收四个数字，增量不保存、不作为下单依据。M5.3a 新网页以 `stream_version: 1` 接入 RPC/SSE，临时文本和工具状态在失败/停止时清除，final + done + 正常 EOF 后才展示权威方案；旧请求保留 unary 阶段 SSE，不因流中失败自动重跑。增加一次受限模型调用，真实费用/时延未实测，详见 [HTTP 协议与兼容边界](docs/agent.md#106-m53a-网关与网页版本化接入)。
...
- M6.3a1 提供单独的 `dev-records` 本机开发库入口：默认只读预检，显式选择库/已有账号后才保存两轮演示历史；重复执行严格重放，不清库、不迁移、不调用外部模型。2026-09-20 用户最新要求改用新建本机库，已在 `127.0.0.1:15432/budgetmatch_agent_dev` 保留 1 个演示账号、1 个会话和 2 轮记录；独立配置与启动方式见[本机保留库联调](docs/agent.md#1110-m63a2-新建本机保留库与真实-rpc-联调)。不复用或覆盖 `.env` 的清理性测试 DSN。
- M6.3a2 的提交后换连接确认、同 ID 重放、独立只读复查及新建数据库正常重启后复查均通过；真实 Agent 二进制的 10 项 TCP RPC 读取/重放/身份隔离检查通过，测试 JWT 为本地签发，不代表 HTTP 登录或页面验收。数据库保留运行，临时 RPC 已停止，商品仍为规则演示 Mock。另保留显式授权与 `verify-full` TLS 的[远程联调入口](docs/agent.md#118-m63a2-显式远程目标与-tls-连接保护)；本轮未访问远程，历史连接失败不能被本机成功覆盖。
...
- 接口、会话、评测口径与分步计划统一见 [Agent 开发文档](docs/agent.md)（M2 按用户决定收尾、独立复核后置；M3 本地安全/检索实验已交付，默认向量优先。M4.1～M4.3a、M5.1～M5.3b 本地实现、M6.1 隔离故障验证及 M6.2 限定范围真实存储验收已交付，M5.3a 推荐页 26 项交互已通过。`/intent/execute` 默认关闭，未在业务环境迁移、回填或启用，新旧推荐仍拒绝规划会话。下一步 M6.3 供应商/代理/版本回滚与最终报告，需另行确认调用和部署范围；真实业务分类数据、向量效果及独立人工复核仍未完成）。
