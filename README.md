...
- [硅基流动 BGE-M3 配置](docs/agent.md#217-硅基流动-bge-m3-配置与兼容边界)已完成本机最小接口验证：`EMBEDDING_DIMENSIONS=1024`，修复固定维度模型不能发送远端 `dimensions` 参数的问题；不等于真实商品库、pgvector 或批量检索效果已验收。
...
- 接口、会话、评测口径与分步计划统一见 [Agent 开发文档](docs/agent.md)（M2 按用户决定收尾、独立复核后置；M3 本地安全/检索实验已交付，默认向量优先。M4.1～M4.3a、M5.1～M5.3b 本地实现、M6.1 隔离故障验证及 M6.2 限定范围真实存储验收已交付，M5.3a 推荐页 26 项交互已通过。`/intent/execute` 默认关闭，未在业务环境迁移、回填或启用，新旧推荐仍拒绝规划会话。下一步 M6.3 供应商/代理/版本回滚与最终报告；超出上述本轮模型限额的调用及部署范围需另行确认，真实业务分类数据、向量效果及独立人工复核仍未完成）。
